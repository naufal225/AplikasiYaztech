...
@section('subtitle', 'Manage your expense reimbursements')
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="text-neutral-600">Submit and track your expense reimbursements</p>
            </div>
            <div class="mt-4 sm:mt-0">
                <a href="{{ route('employee.reimbursements.create') }}" class="btn-secondary">
                    ...
                    <tbody class="bg-white divide-y divide-neutral-200">
                        @forelse($reimbursements as $reimbursement)
                            <tr class="hover:bg-neutral-50 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="w-8 h-8 bg-secondary-100 rounded-full flex items-center justify-center mr-3">
                                            <span class="text-secondary-600 font-semibold text-xs">{{ substr($reimbursement->employee->name, 0, 1) }}</span>
                                        </div>
                                        <div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-bold text-neutral-900">{{ $reimbursement->formatted_total }}</div>
                                    @if($reimbursement->customer)
                                        <div class="text-sm text-neutral-500">{{ $reimbursement->customer->name }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-neutral-900">{{ $reimbursement->date->format('M d, Y') }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        <a href="{{ route('employee.reimbursements.show', $reimbursement->id) }}" class="text-primary-600 hover:text-primary-900">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        @if(Auth::id() === $reimbursement->employee_id && $reimbursement->status === 'pending')
                                            <a href="{{ route('employee.reimbursements.edit', $reimbursement->id) }}" class="text-secondary-600 hover:text-secondary-900">
                                        ...
                                        @endif
                                        @if((Auth::id() === $reimbursement->approver_id || Auth::user()->role === 'admin') && $reimbursement->status === 'pending')
                                            <a href="{{ route('employee.reimbursements.review', $reimbursement->id) }}" class="text-success-600 hover:text-success-900">
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="text-neutral-400">
                                        ...
                                        <p class="text-sm">Submit your first expense reimbursement to get started</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
