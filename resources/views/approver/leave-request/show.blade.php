                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-sm text-primary-100">
                                Submitted on {{ \Carbon\Carbon::parse($leave->created_at)->format('M d, Y \a\t H:i') }}
                            </p>
                        </div>
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center p-3 border rounded-lg bg-neutral-50 border-neutral-200">
                                ...
                                @php
                                    $duration = \Carbon\Carbon::parse($leave->date_start)->diffInDays(\Carbon\Carbon::parse($leave->date_end)) + 1;
                                @endphp
                        ...
                        <!-- Approval Notes -->
                        @if($leave->status_1 === 'approved' && $leave->note_1)
                            <div class="space-y-2 md:col-span-2">
                                <label class="text-sm font-semibold text-neutral-700">Approval Notes</label>
                                <div class="p-4 border rounded-lg bg-success-50 border-success-200">
                                    <p class="leading-relaxed text-success-900">{{ $leave->note_1 }}</p>
                                </div>
                            </div>
                        @elseif($leave->status_1 === 'rejected' && $leave->note_1)
                            <div class="space-y-2 md:col-span-2">
                                <label class="text-sm font-semibold text-neutral-700">Rejection Notes</label>
                                <div class="p-4 border rounded-lg bg-error-50 border-error-200">
                                    <p class="leading-relaxed text-error-900">{{ $leave->note_1 }}</p>
                                </div>
                            </div>
                        @endif
                    </div>
                ...
                <div class="p-6 space-y-3">
                    ...
                    @if(Auth::id() === $leave->employee_id && $leave->status_1 === 'pending')
                        ...
                        <button
                            ...
                            data-leave-name="Leave Request #{{ $leave->id }}"
                            title="Delete">
                        ...
                        <form id="delete-form-{{ $leave->id }}"
                              action="{{ route('approver.leaves.destroy', $leave->id) }}"
                              method="POST"
                              style="display: none;">
                            @csrf
                            @method('DELETE')
